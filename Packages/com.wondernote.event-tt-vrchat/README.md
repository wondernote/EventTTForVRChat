...
このパッケージは、VRChatで利用可能なイベントカレンダー（旧：タイムテーブル）機能を提供する一連のユニティアセットを含んでいます。これにより、VRChatのユーザーはイベント情報を簡単に追跡できるようになります。

## パッケージの特徴
- イベント画像が時間帯ごとに並び、それぞれの画像をクリックすると、別ウィンドウで詳細情報が表示されます。
- ユーザーフレンドリーなインターフェイスにより、情報を視覚的かつ直感的に理解しやすくなっています。
- ウェブサイトと連動し、1日に4回（1時、7時、13時、19時）自動で更新されます。
- 表示したいイベントを事前に選ぶことができます（すべての場合：直近3日分 / 選択した場合：6か月分）。
